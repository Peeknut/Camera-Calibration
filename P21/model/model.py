...
import math
from model.model_part import conv2d
from model.model_part import fc
...
def inference(images, reuse=False, trainable=True):
    ...
    coarse2 = tf.nn.max_pool(coarse2_conv, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID', name='pool2')
    ...
    coarse5_conv = conv2d('coarse5', coarse4, [3, 3, 384, 256], [256], [1, 1, 1, 1], padding='SAME', reuse=reuse, trainable=trainable)
    coarse5 = tf.nn.max_pool(coarse5_conv, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool3')
...
def loss(logits, depths, invalid_depths):
    ...
    predict = tf.multiply(logits_flat, invalid_depths_flat) # 点乘
    target = tf.multiply(depths_flat, invalid_depths_flat)

    d = tf.subtract(predict, target)
    square_d = tf.square(d) # 对每个元素进行平方
    sum_square_d = tf.reduce_sum(square_d, 1) # 对1维进行求和，即每一行元素相加
