...
class Dataset:
    ...
    def __init__(self, settings, patch_locations, phase):
        ...
        # data_path = data/kitti_2015/training
    ...
    def _parse_function(self, sample_info):
        ...
        idx = tf.to_int32(sample_info[0]) # 图片在图片名列表中的索引下标
        left_center_x = tf.to_int32(sample_info[1]) # 左图的列号
        left_center_y = tf.to_int32(sample_info[2]) # 左图的行号
        right_center_x = tf.to_int32(sample_info[3]) # 右图的列号

        left_image = self.left_images[idx]
        right_image = self.right_images[idx]

        # left_patch的size是（patch_size, patch_size）,中心点是(y,x)
        left_patch = left_image[left_center_y -\
                                ...
                                self._settings.half_patch_size + 1, :]
        # right_patch的size是（patch_size, patch_size+disparity_range）,中心点是(y, x-leftimage(y,x))
