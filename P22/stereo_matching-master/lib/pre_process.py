...
def _compute_valid_locations(disparity_image_paths, sample_ids, img_height,
    ...
    '''
    参数：
        disparity_image_paths：标准视差图片名字列表
        sample_idx：下表索引列表
    ...
    for i, idx in enumerate(sample_ids):
        ...
        disp_img /= 256
        row_locs, col_locs = np.where(disp_img != 0) # 合法视差的坐标（行，列）
...
def find_and_store_patch_locations(settings):
    ...
    train_ids = sample_indices[0:settings.num_train] # train图片的序号
    val_ids = sample_indices[settings.num_train:settings.num_val+settings.num_train]

    # Training set.
    # valid_locations_train = 图片在train/vaild中的索引编号，左图中心点坐标x列，左图中心点坐标y行，右图中心点坐标x列(=左图中心点坐标x列-左图(y,x)像素值)
